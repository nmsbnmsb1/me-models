...
class DBUtils {
    ...
    static getTableConfig(name, fields, index = {}, nsdata, nslist = {}, nsref = {}) {
        let all = {
            name,
    ...
    static getFieldsObject(alias = '', fields, defaultAs, sp = false, customAs, hget, hgetOptions) {
        let tfs = fields;
        if (think_helper_1.default.isString(fields) && fields !== '*')
            tfs = fields.split(',');
    ...
    static getDRObject(options) {
        ...
        let ns;
        if (nslis) {
            if (think_helper_1.default.isString(nslis))
                ns = { ...fs[0].t.nslist[nslis] };
            else if (nslis.t) {
                let { t, name } = nslis;
                ns = { ...t.nslist[name] };
            }
